...
def post_user():
    ...
    if (request.json.get('first_name')):
        new_member["first_name"] = request.json.get("first_name")
    else:
        return jsonify({"msg": "Nombre es requerido"}), 400

    if (request.json.get("last_name")):
        return jsonify({"msg": "Apellido no es requerido"}), 400

    if (request.json.get("age")):
        new_member["age"] = request.json.get("age")
    else:
        return jsonify({"msg": "Edad es requerida"}), 400

    if (request.json.get("lucky_numbers")):
        new_member["lucky_numbers"] = request.json.get("lucky_numbers")
    else:
        return jsonify({"msg": "Lucky Numbers requerido!"}), 400
